...
import { EnvelopeIcon, TrashIcon, PlusIcon, ArrowLeftIcon } from "@heroicons/react/24/outline";
...
export default function AdminSubscribersPage() {
  ...
  const fetchSubscribers = async () => {
    try {
      const res = await adminFetch(`${API_URL}/api/subscribers/`);
  ...
  const handleAdd = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Adding a recipient from the admin side doesn't need admin auth on
      // the backend (it's the same public subscribe endpoint), but routing
      // it through adminFetch here is harmless and keeps this page's error
      // handling uniform.
  ...
  const handleRemove = async (subscriber) => {
    ...
    try {
      const res = await adminFetch(`${API_URL}/api/subscribers/${subscriber.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP error! Status: ${res.status}`);
  ...
  return (
    <div className="p-6 md:p-10 max-w-2xl mx-auto">
      <Link href="/dashboard/admin" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 mb-4">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-semibold text-slate-900">Subscribers</h1>
        <p className="text-slate-500 mt-1">
          Everyone below gets an email digest whenever new consulting opportunities are found. This list is admin-only.
        </p>
      </div>
